...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('emsttr_sync.log'),
...
class Config:
    def __init__(self):
        ...
        if self.is_windows:
            ...
            self.db_path = r"C:\Users\Administrator\Desktop\TradingData_STR_AC2.db"
        else:
            self.credentials_path = r"/home/bryan_oosterveld/credentials/bigquery_credentials_placeholder.json"
            self.db_path = r"/home/bryan_oosterveld/TradingData_STR_AC2.db"
...
class PositionMirror:
    ...
    def invert_order(self, position):
        """Invert a single position/order for sell-the-rip strategy."""
        inverted_position = position.copy()
        
        # Invert order type
        if position.get('Type') == 'BUY':
            inverted_position['Type'] = 'SELL'
        elif position.get('Type') == 'SELL':
            inverted_position['Type'] = 'BUY'
        
        # Swap TP and SL levels
        original_tp = position.get('CurrentTP')
        original_sl = position.get('CurrentSL')
        
        if original_tp and original_sl:
            inverted_position['CurrentTP'] = original_sl  # Original SL becomes new TP
            inverted_position['CurrentSL'] = original_tp  # Original TP becomes new SL
            
            logging.info(f"Inverted {position.get('Symbol', 'Unknown')} order: "
                        f"{position.get('Type')} -> {inverted_position['Type']}, "
                        f"TP: {original_tp} -> {inverted_position['CurrentTP']}, "
                        f"SL: {original_sl} -> {inverted_position['CurrentSL']}")
        
        return inverted_position
    
    def fetch_and_invert_positions(self):
        """Fetch current positions and invert them for sell-the-rip strategy."""
        try:
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
            ...
            cursor.execute('''
                SELECT Type, Label, Symbol, Volume, EntryPrice, CurrentPrice, CurrentSL, CurrentTP
                FROM Positions
            ...
            for pos in positions:
                ...
                # Invert the position
                inverted_pos = self.invert_order(position_dict)
                inverted_positions.append(inverted_pos)
                
                # Store in mirrored_positions for tracking
                self.mirrored_positions[pos[2]] = inverted_pos  # Use Symbol as key
            
            conn.close()
            logging.info(f"Fetched and inverted {len(inverted_positions)} positions")
            return inverted_positions
            
        except Exception as e:
            logging.error(f"Error fetching and inverting positions: {e}")
            return []
    ...
    def mirror_positions(self, current_price, signal_files=None):
        ...
        # Fetch and invert existing positions
        inverted_positions = self.fetch_and_invert_positions()
        ...
        if signal_files:
            ...
            if not isinstance(signal_files, dict):
                # If not provided as a dict, create a dummy dict with "SELL" values
                signal_files_dict = {f"signal_file_{i}": "SELL" for i in range(len(signal_files))}
            else:
    ...
    def check_portfolio_tp_sl(self, current_price):
        ...
        # For sell-the-rip strategy: TP when price goes DOWN, SL when price goes UP
        if percent_change <= -TAKE_PROFIT_PERCENTAGE:
            logging.info(f"Portfolio take profit hit: Current price {current_price:.2f} is {abs(percent_change):.2f}% below entry price {self.entry_price:.2f}")
            ...
            logging.info(f"Portfolio stop loss hit: Current price {current_price:.2f} is {percent_change:.2f}% above entry price {self.entry_price:.2f}")
            self.exit_price = current_price
            self.mirroring_active = False
            ...
            while self.running:
                ...
                try:
                    ...
                    if timestamp and price:
                        ...
                        # Check for rip threshold and that mirroring is not already active
                        if self.ohlc_calculator.check_rip_threshold() and not self.position_mirror.mirroring_active:
                            # Log that a rip has been detected
                            ...
                            signal_files = {
                                "strategy_one.py": "SELL",
                                "strategy_two.py": "SELL",
                                "strategy_three.py": "BUY"
                            }
                    ...
                    if current_candle['open'] is not None:
                        ...
                        if self.position_mirror.mirroring_active and random.random() < 0.05:  # 5% chance each cycle
                            ...
                            if self.position_mirror.signal_handler.watched_signals:
                                ...
                                new_value = "BUY" if current_value == "SELL" else "SELL"
