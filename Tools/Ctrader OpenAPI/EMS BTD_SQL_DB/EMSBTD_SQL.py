...
def log_to_sqlite(event_type, message):
    db_path = r'C:\Users\Administrator\Desktop\EMS_Log_AC2.db'
    try:
...
class OHLCCalculator:
    def __init__(self, db_path=r"C:\Users\Administrator\Desktop\TradingData_BTD_AC2.db"):
    ...
    def check_dip_threshold(self):
        ...
        if self.dip_already_triggered:
            ...
            log_to_sqlite('INFO', "Dip already triggered in this session, no more dip buying allowed")
            return False
        ...
        if dip_percentage >= DIP_THRESHOLD_PERCENTAGE:
            ...
            log_to_sqlite('INFO', f"Dip threshold detected: {dip_percentage:.2f}% (threshold: {DIP_THRESHOLD_PERCENTAGE}%)")
...
class PositionMirror:
    def __init__(self, pnl_log_path=r"C:\Users\Administrator\Desktop\Sonixen\Tools\Ctrader OpenAPI\EMS BTD_SQL_DB\Pnl_Log.py"):
    ...
    def mirror_positions(self, current_price, strategies):
        ...
        # Mirror positions with volumes from StrategyConfigAC2
        for position in positions:
            ...
            if label in strategy_configs:
                ...
                self.mirrored_positions[label] = position_copy
            else:
                # If not found in strategy config, use as is
                self.mirrored_positions[label] = position
                logging.warning(f"Strategy config not found for {label}, using original volume: {position.get('Volume')}")
            
        # Mirror open orders with volumes from StrategyConfigAC2
        for order in orders:
            ...
            if label in strategy_configs:
                ...
                self.mirrored_orders[label] = order_copy
            else:
                # If not found in strategy config, use as is
                self.mirrored_orders[label] = order
...
class TradingSystem:
    ...
    async def monitor_share_price(self):
        """Monitor WeightedSharePrice from TradingData_BTD_AC2.db and check TP/SL"""
        while True:
            try:
                # Fetch latest data
                ...
                if timestamp and price:
                    ...
                    # If mirroring is not active, check if dip threshold is met
                    if not self.mirroring_active and self.ohlc_calculator.check_dip_threshold():
                        logging.info("Dip threshold met, starting position mirroring")
                        log_to_sqlite('INFO', "Dip threshold met, starting position mirroring")
                        ...
                        if self.position_mirror.mirror_positions(price, self.strategies):
                            ...
                            for label, position in self.position_mirror.mirrored_positions.items():
                                ...
                                # Create order data to add to pending orders
                            ...
                            for observer in self.observers:
                                if not observer.is_alive():
                                    observer.start()
                            logging.info("Signal watchers started - dip session is now active")
                            log_to_sqlite('INFO', "Signal watchers started - dip session is now active")
                    ...
                    # If mirroring is active, check portfolio-wide TP/SL
                        ...
                        for observer in self.observers:
                            if observer.is_alive():
                                observer.stop()
                                observer.join()
                        logging.info("Signal watchers stopped - dip session ended")
                        log_to_sqlite('INFO', "Signal watchers stopped - dip session ended")
    ...
    def start_signal_monitoring(self):
        ...
        logging.info("Signal watchers initialized but not started - waiting for active dip session")
        log_to_sqlite('INFO', "Signal watchers initialized but not started - waiting for active dip session")
